...
Once the application is complete, all associated PDF files are stored in the document management system d.3 and a file for the application is created. The examination office then defines corresponding tasks for the specialist representatives in d.3.

The expert representatives handle these tasks sequentially. For this purpose, they refer to the complete file containing all documents belonging to the application and to the files stored in d.3, to find the parts relevant to them in all files and to document the result of their check in a Text file on a shared network drive. Only when one expert representative has completed their exam will the next one be informed. Every The expert representative receives access to all submitted documents.

After all exams have been completed, the examination office uses a Java program to create a PDF document from the text file, which contains the exam results summarized. This document will then be sent to the student via email.

Unfortunately, the current implementation of the process presents some problems, of which we will only mention a few. The examination office can only provide documents Request further information, but do not reject individual module applications of an application as incomplete. The expert representatives only check the modules assigned to them, However, they see the full application. In particular, they search the files containing all module and performance descriptions for the parts relevant to them out. The result of the application review is written to a text file on a network drive. The results cannot be directly analyzed in Document management system d.3 will be entered.

---

https://www.fernuni-hagen.de/wirtschaftswissenschaft/studium/download/antrag_1-2.pdf

https://www.fernuni-hagen.de/dekwiwi/tr